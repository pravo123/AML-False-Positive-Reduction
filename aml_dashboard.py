"""
AML False Positive Reduction System
...
1. Synthetic AML transaction data generation
2. Advanced feature engineering for behavioral analysis
3. Ensemble ML models (XGBoost, LightGBM, Random Forest)
4. SHAP explainability for regulatory compliance
5. Custom evaluation metrics for AML use cases
"""
...
class AMLDataGenerator:
    ...
    def _generate_alerts(self, df):
        ...
        # Rule 1: Large transactions (many false positives from legitimate business)
        ...
        # Rule 3: Round amounts (structuring concern)
    ...
    def _generate_ground_truth(self, df):
        ...
        if len(alerted_idx) > 0:
            ...
            for idx in alerted_idx:
                ...
                # True AML if risk score is high
                if risk_score >= 6:
                    true_aml[idx] = 1
                elif risk_score >= 4:
                    # Some randomness for realistic data
...
class AMLFeatureEngineering:
    ...
    def create_features(self, df):
        ...
        # Customer-level aggregations (behavioral patterns)
    ...
    def prepare_model_data(self, df):
        ...
        # Only use alerted transactions for training
...
class AMLModelTrainer:
    """Train and evaluate ensemble models for AML false positive reduction"""
    ...
    def train_models(self, X_train, X_test, y_train, y_test, feature_names):
        ...
        for name, model in self.models.items():
            ...
            total_alerts = len(y_test)
            false_positives = cm[0, 1]
            true_negatives = cm[0, 0]
            fp_reduction = (true_negatives / (true_negatives + false_positives)) * 100
    ...
    def plot_precision_recall_curves(self, y_test):
        ...
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig('/mnt/user-data/outputs/precision_recall_curves.png', dpi=300, bbox_inches='tight')
        plt.close()


def calculate_business_impact(y_test, y_pred, avg_investigation_time_hours=2):
    """Calculate business impact of false positive reduction"""
    cm = confusion_matrix(y_test, y_pred)
    tn, fp, fn, tp = cm.ravel()

    baseline_alerts = len(y_test)
    reduced_alerts = fp + tp  # Alerts that would still be investigated
    eliminated_fps = tn  # False positives correctly identified
...
def main():
    ...
    # 7. Business Impact (using best model - XGBoost)
    ...
    summary = []
    summary.append("=" * 80)
    summary.append("AML FALSE POSITIVE REDUCTION - PROJECT SUMMARY")
    summary.append("=" * 80)
    ...
    print("\n" + summary_text)
    print("\nProject artifacts saved to /mnt/user-data/outputs/")
    print("Files created:")
